...
import { getOneDriveItemMetadata, GraphApiError } from '@/lib/microsoft-graph/client'
...
// Links a OneDrive file into DocTrack -- stores a reference (item id,
// web link, and a metadata snapshot), never copies the file's bytes
// into our own storage. No document_versions row: OneDrive is the
// versioning authority for a linked document, not this app.
export async function POST(request: Request) {
  ...
  const { file_id, title, matter_id, category } = await request.json()
  if (!file_id || typeof title !== 'string' || !title.trim()) {
    return NextResponse.json({ error: 'file_id and title are required' }, { status: 400 })
  }
  ...
  let metadata
  try {
    metadata = await getOneDriveItemMetadata(accessToken, file_id)
  } catch (err) {
    const message = err instanceof GraphApiError ? err.message : 'Could not read file metadata from OneDrive'
  ...
  await supabaseAdmin.from('document_events').insert({
    ...
    user_id: user.id,
    event_type: 'created',
    metadata: { source: 'onedrive', filename: metadata.name },
  })
