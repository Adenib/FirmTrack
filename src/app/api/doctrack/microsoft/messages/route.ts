...
import { listOutlookMessages, GraphApiError } from '@/lib/microsoft-graph/client'
...
export async function GET(request: Request) {
  ...
  try {
    const messages = await listOutlookMessages(accessToken, q)
    return NextResponse.json({ messages })
  } catch (err) {
    const message = err instanceof GraphApiError ? err.message : 'Could not browse Outlook'
