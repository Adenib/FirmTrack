...
describe('auto-posting on Phase 1 write routes', () => {
  ...
  it('disbursement: Dr client_costs_advanced / Cr operating_cash', async () => {
    const res = await tenant.fetch('/api/accounttrack/disbursements', {
      method: 'POST',
      body: JSON.stringify({ matter_id: matterId, description: 'Filing fee', amount: 150 }),
    })
    ...
    expect(findLine(lines, 'client_costs_advanced').debit).toBe(150)
    expect(findLine(lines, 'operating_cash').credit).toBe(150)
  })

  it('trust deposit: Dr trust_bank / Cr trust_liability', async () => {
    const res = await tenant.fetch('/api/accounttrack/trust-ledger', {
      method: 'POST',
      body: JSON.stringify({ matter_id: matterId, ledger_type: 'trust', amount: 500, description: 'Deposit' }),
    })
    expect(res.status).toBe(200)
    const { entry } = await res.json()

    const lines = await getLinesForSource(tenant.tenantId, 'trust_deposit', entry.id)
    expect(findLine(lines, 'trust_bank').debit).toBe(500)
    expect(findLine(lines, 'trust_liability').credit).toBe(500)
  })

  it('trust withdrawal: Dr trust_liability / Cr trust_bank', async () => {
    const res = await tenant.fetch('/api/accounttrack/trust-ledger', {
      method: 'POST',
      body: JSON.stringify({ matter_id: matterId, ledger_type: 'trust', amount: -200, description: 'Withdrawal' }),
    })
    expect(res.status).toBe(200)
    const { entry } = await res.json()

    const lines = await getLinesForSource(tenant.tenantId, 'trust_withdrawal', entry.id)
    expect(findLine(lines, 'trust_liability').debit).toBe(200)
    expect(findLine(lines, 'trust_bank').credit).toBe(200)
  })

  it('retainer deposit: Dr trust_bank / Cr retainer_liability', async () => {
    const res = await tenant.fetch('/api/accounttrack/trust-ledger', {
      method: 'POST',
      body: JSON.stringify({ matter_id: matterId, ledger_type: 'retainer', amount: 300, description: 'Retainer' }),
    })
    expect(res.status).toBe(200)
    const { entry } = await res.json()

    const lines = await getLinesForSource(tenant.tenantId, 'retainer_deposit', entry.id)
    expect(findLine(lines, 'trust_bank').debit).toBe(300)
    expect(findLine(lines, 'retainer_liability').credit).toBe(300)
  })

  it('retainer withdrawal: Dr retainer_liability / Cr trust_bank', async () => {
    const res = await tenant.fetch('/api/accounttrack/trust-ledger', {
      method: 'POST',
      body: JSON.stringify({ matter_id: matterId, ledger_type: 'retainer', amount: -100, description: 'Draw down' }),
    })
    expect(res.status).toBe(200)
    const { entry } = await res.json()

    const lines = await getLinesForSource(tenant.tenantId, 'retainer_withdrawal', entry.id)
    expect(findLine(lines, 'retainer_liability').debit).toBe(100)
    expect(findLine(lines, 'trust_bank').credit).toBe(100)
  })
  ...
  it('invoice creation: Dr accounts_receivable / Cr fees_earned', async () => {
    const entryRes = await tenant.fetch('/api/timetrack/entries', {
      method: 'POST',
      body: JSON.stringify({
        entries: [{ matter_id: matterId, hours: 4, rate: 100, amount: 400, billable: true }],
      }),
    })
    ...
    expect(Number(invoice.total_amount)).toBe(400)

    const lines = await getLinesForSource(tenant.tenantId, 'invoice_created', invoice.id)
    expect(findLine(lines, 'accounts_receivable').debit).toBe(400)
    expect(findLine(lines, 'fees_earned').credit).toBe(400)
    ...
    const payRes = await tenant.fetch('/api/accounttrack/invoices', {
      method: 'PATCH',
      body: JSON.stringify({ id: invoice.id, payment_amount: 400 }),
    })
    ...
    expect(findLine(paymentLines, 'operating_cash').debit).toBe(400)
    expect(findLine(paymentLines, 'accounts_receivable').credit).toBe(400)
  ...
  it('invoice void (unpaid): Dr fees_earned / Cr accounts_receivable', async () => {
    ...
    expect(findLine(lines, 'fees_earned').debit).toBe(200)
    expect(findLine(lines, 'accounts_receivable').credit).toBe(200)
  })
})
